...
def create_app() -> Flask:
        ...
        for probe in probes:
            try:
                ...
                if resp.status_code < 400:
                    return {"url": url, "status": "healthy", "code": resp.status_code, "probe": probe}
    ...
    @app.get("/internal/services/health")
    def internal_services_health():
        results = {}
        for key in ("PROMETHEUS_URL", "LOKI_URL"):
            url = getattr(_cfg, key, "")
            if not url:
    ...
    if os.getenv("ENABLE_FLASK_WEBHOOKS", "").lower() in ("1", "true", "yes"):
        _register_webhook_routes(app)

    return app


def _register_webhook_routes(app: Flask) -> None:
