...
class TestEnvironments(unittest.TestCase):
    def setUp(self):
        ...
        for loader, data in (
            ("list_endpoints", _ENDPOINTS["endpoints"]),
            ("list_environments", _ENVIRONMENTS["environments"]),
        ):
            p = mock.patch.object(env._settings_store, loader, return_value=data)
            p.start()
    ...
    def test_no_environments_uses_boot_defaults(self):
        env.reset_cache()
        with mock.patch.object(env._settings_store, "list_environments", return_value=[]):
            e = env.resolve("prod")
