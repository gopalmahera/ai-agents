...
export function logEntry(f: fs.Dirent | { name: string; isFile: () => boolean }, stat: fs.Stats) {
  const name = f.name;
...
export function listLogs(q = "", type = "", limit = 100) {
  const logsDir = path.resolve(LOGS_DIR);
  if (!fs.existsSync(logsDir)) return [];
  const files = fs
    .readdirSync(logsDir, { withFileTypes: true })
    .filter((f) => f.isFile())
    .map((f) => ({ f, stat: fs.statSync(path.join(logsDir, f.name)) }))
    .sort((a, b) => b.stat.mtimeMs - a.stat.mtimeMs);

  const entries = [];
  for (const { f, stat } of files) {
    const entry = logEntry(f, stat);
    if (q && !entry.alertname.toLowerCase().includes(q) && !f.name.toLowerCase().includes(q)) continue;
...
export function readLog(filename: string) {
  if (!SAFE_NAME_RE.test(filename)) throw new Error("Invalid filename");
  const logFile = path.join(path.resolve(LOGS_DIR), filename);
  if (!fs.existsSync(logFile)) throw new Error("Not found");
  return fs.readFileSync(logFile, "utf-8");
}

export function deleteLog(filename: string) {
  if (!SAFE_NAME_RE.test(filename)) throw new Error("Invalid filename");
  const logFile = path.join(path.resolve(LOGS_DIR), filename);
  if (!fs.existsSync(logFile)) throw new Error("Not found");
