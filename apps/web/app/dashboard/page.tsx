...
export default function DashboardPage() {
  const { data: stats, isLoading, refetch, isFetching } = useQuery({
    ...
    queryFn: () => api.get<MetricsStats>("/api/v1/metrics/stats"),
    refetchInterval: 60_000,
  });

  const { data: health } = useQuery({
    queryKey: ["mcp-health"],
    queryFn: () => api.get<McpHealth>("/api/v1/health/mcp"),
    staleTime: Infinity,
  });
  ...
  const { data: platformHealth } = useQuery({
    ...
    queryFn: () =>
      api.get<{ status: string; mongo: boolean; redis: boolean }>("/api/v1/health").catch(() => ({
        status: "unavailable",
        mongo: false,
        redis: false,
      })),
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        <div className="card flex items-center gap-3">
          <Database size={14} className={platformHealth?.redis ? "text-emerald-500" : "text-red-500"} />
          <div>
            <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Redis / Mongo</p>
            <p className="text-xs text-slate-400 dark:text-slate-500">
              {platformHealth?.redis
                ? `Redis OK${platformHealth.mongo ? " · Mongo OK" : " · Mongo unavailable"}`
                : "Unavailable — metrics may be incomplete"}
            </p>
          </div>
        </div>
      </div>
