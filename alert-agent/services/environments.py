...
Both the endpoint registry (``config:endpoints_yaml``) and the environment map
(``config:environments_yaml``) load from Redis (shared + synced across replicas)
...
def _load_endpoints() -> dict[str, dict]:
    ...
    data = _load_raw(_redis.endpoints_yaml_load, path)
    index: dict[str, dict] = {}
...
def _load_environments() -> dict[str, dict]:
    ...
    data = _load_raw(_redis.environments_yaml_load, path)
    index: dict[str, dict] = {}
