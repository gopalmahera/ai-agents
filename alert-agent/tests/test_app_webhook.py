...
class TestAppWebhook(unittest.TestCase):
    ...
    @patch.object(webhook_controller, "threading")
    def test_firing_does_not_send_status_starts_investigation(
        self, mock_threading, mock_slack, mock_redis, _mock_log
    ):
    ...
    @patch.object(webhook_controller, "send_alert_status")
    @patch.object(webhook_controller, "threading")
    def test_resolved_sends_status_only(self, mock_threading, mock_slack, mock_redis, _mock_log):
